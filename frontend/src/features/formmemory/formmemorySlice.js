...
export const postAuthCode= createAsyncThunk('formMemory/postAuthCode', 
  async (postData, thunkAPI) => {
      const response = 
        await axios.post('https://estimating-database.dowbuilt.io/restapi/userdata/', postData)
      return response.data 
  }
)

export const getUserData= createAsyncThunk('formMemory/getUserData', 
  async (pk, thunkAPI) => {
      console.log('pk variable in request:', pk)
      const response = 
        await axios.get(`https://estimating-database.dowbuilt.io/restapi/userdata/${pk}/`)
      return response.data
    //.userData
  }
)
...
export const fetchFormMemory= createAsyncThunk('formMemory/fetchFormMemory',
    async () => {
    const response = 
      await client_copy.get('http://localhost:9000/output')
    return response
})
...
export const formMemorySlice = createSlice({
    ...
    reducers: {
        ...
        updatepk(state,action){
          state.userData.pk = action.payload
        },
    },
  extraReducers: {
    // intakeData
    ...
    [getUserData.fulfilled]: (state, action) => {
      ...
      console.log('get request success, payload:', action.payload)
      // Add any fetched posts to the array
      state.userData.heap = JSON.parse(action.payload)
    },
    [getUserData.rejected]: (state, action) => {
      state.userData.status = 'failed'
      state.userData.error = action.error.message
    },
    [postAuthCode.fulfilled]: (state, action) => {
      console.log('post request success, payload:', action.payload)
      state.userData.pk=action.payload.pk
    },
  }
